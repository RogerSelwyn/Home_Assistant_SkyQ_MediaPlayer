...
The switches are generated as `skyq<room>.yaml` in the `<config_root>/templates` directory, which should be included in your configuration using a `!include_dir` function. Details on the use of these functions can be found here - [Splitting up the configuration](https://www.home-assistant.io/docs/configuration/splitting_configuration/)
